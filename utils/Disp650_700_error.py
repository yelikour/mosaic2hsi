...
save_path = '/home/wangruozhang/mosaic2hsi/TestOutput1/Compare650_700'
...
for gt_file, output_file in tqdm(zip(gt_files, output_files), total=len(gt_files), desc="Processing files"):
    ...
    # 提取倒数6个通道
    gt_last_6_channels = gt_data[:, :, -6:]
    output_last_6_channels = output_data[:, :, -6:]
    ...
    for i in range(6):
        # 计算差异
        difference = output_last_6_channels[:, :, i] - gt_last_6_channels[:, :, i]

        # 使用颜色映射显示 GT 和差异
        axs[0, i].imshow(gt_last_6_channels[:, :, i], cmap='gray')
