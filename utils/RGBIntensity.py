...
def plot_combined_intensity_curve(marked_image_path_2, npy_file, rgb_curve_path, point, save_folder):
    """
    ...
    """
    # # 加载 RGB 图像
    rgb_image = tifffile.imread(marked_image_path_2)

    # 加载npy文件
    hyperspectral_image = np.load(npy_file)

    # Display the image and mark the point at (1000, 850)
    plt.figure(figsize=(10, 8))
    plt.imshow(rgb_image)

    x, y = point

    # Mark the point at (1000, 850)
    plt.plot(x, y, 'ro')  # red dot to mark the spot
    plt.title(f"Marked Pixel at {point}")

    # Save and show the image with the marked point
    plt.show()
...
def main():
    marked_image_path_2 = r'../Output/RGB/output_1.tiff'
    npy_file = r'../Output/npy/output_1.npy'
    rgb_curve_path = '../RGBTransCurve'
    point = (630, 630)  # 选择一个点
    save_folder = '../TestOutput/Fig'

    plot_combined_intensity_curve(marked_image_path_2, npy_file, rgb_curve_path, point, save_folder)
