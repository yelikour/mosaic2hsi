...
class MyDataSet(torch.utils.data.Dataset):
    def __init__(self, dataPath, transform=None):
        ...
        # 过滤出所有包含RGB64的.tiff文件
        self.input_paths = sorted([os.path.join(root, f)
                                   for root, _, files in os.walk(dataPath)
                                   for f in files if 'RGB64' in f and f.endswith('.tiff')])
    ...
    def __getitem__(self, idx):
        ...
        # 读取RGB64tiff文件
        RGBdata = tifffile.imread(input_path)

        # 转成mosaic
        input_data = create_mosaic(RGBdata)
...
def main():
    ...
    save_npy_folder = '/home/wangruozhang/mosaic2hsi/Output64/npy'
    output_rgb_folder = '/home/wangruozhang/mosaic2hsi/Output64/RGB'
